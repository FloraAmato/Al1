...
namespace CreaProject.Pages.Disputes
{
    public class EditModel : DisputeBasePageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int id)
        {
            ...
            var isMediator = await AuthorizationService.AuthorizeAsync(
                User,
                Dispute,
                DisputeOperations.Update
            );
            var isAgent = await AuthorizationService.AuthorizeAsync(
                User,
                Dispute,
            ...
            {
                var agentsThatBidId = Dispute
                    .AgentUtilities.Select(d => d.AgentId)
                    .Distinct()
                    .ToList();
                var agents = Dispute.Agents.Select(a => a.AgentId).ToList();
                var allBid =
                    agentsThatBidId.All(agents.Contains) && agentsThatBidId.Count == agents.Count;
        ...
        public async Task<IActionResult> OnPostAsync(int id)
        {
            ...
            if (dispute.Status.Equals(DisputeStatus.SettingUp))
                dispute.ResolutionMethod = Dispute.ResolutionMethod;
        ...
        public async Task<IActionResult> OnPostStartBiddingAsync(int id)
        {
            ...
            if (
                Dispute.Agents.Count > 1
                && Dispute.Goods.Count > 1
            ...
            )
            {
                ...
                {
                    TempData["ErrorShareSum"] =
                        "The sum of Share of entitlement is not equal to 100%";
                }
            }
